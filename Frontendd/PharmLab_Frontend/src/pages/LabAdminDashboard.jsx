...
export default function LabAdminDashboard() {
  ...
  useEffect(() => {
    const normalizedCas = newItem.casNumber.trim();
    if (!createOpen || !normalizedCas || autofillingCas || normalizedCas === lastAutofilledCas) return;

    const timerId = setTimeout(() => {
      autofillFromCas(newItem, setNewItem, setAutofillingCas, setLastAutofilledCas, setCasLookupMessage, setCasLookupType);
    }, 500);

    return () => clearTimeout(timerId);
  }, [autofillingCas, createOpen, lastAutofilledCas, newItem]);

  useEffect(() => {
    const normalizedCas = editItem.casNumber.trim();
    if (!editOpen || !normalizedCas || editAutofillingCas || normalizedCas === lastEditAutofilledCas) return;

    const timerId = setTimeout(() => {
      autofillFromCas(editItem, setEditItem, setEditAutofillingCas, setLastEditAutofilledCas, setEditCasLookupMessage, setEditCasLookupType);
    }, 500);

    return () => clearTimeout(timerId);
  }, [editAutofillingCas, editItem, editOpen, lastEditAutofilledCas]);
    ...
    <div className='rounded-xl border border-[#d9e1ca] bg-[#f7f8f1] p-4 dark:border-[#414a33] dark:bg-[#28301f]'>
      <div className='flex flex-col gap-3 sm:flex-row sm:items-end'>
        ...
        <Button type='button' variant='outline' className='sm:w-auto' onClick={() => autofillFromCas(item, setItem, setLoading, setLastFetchedCas, setStatusMessage, setStatusType)} disabled={isLoadingCas}>
          {isLoadingCas ? 'Fetching...' : 'Autofill From PubChem'}
        </Button>
      </div>
      <p className='mt-2 text-xs text-[#71805a] dark:text-[#c5d0b5]'>Enter the CAS number and available PubChem fields will auto-fill after a short pause. This only fills the form. Use Save chemical to add it to inventory.</p>
